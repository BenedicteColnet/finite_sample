...
for (sample.size in c(150, 300, 500, 1000)){
  ...
  for (i in 1:50){
    ...
    for (method in different_subset_tested){
      ...
      simulation.to.estimate$mu.hat.1 <- mu.hat.1
      simulation.to.estimate$mu.hat.0 <- mu.hat.0
      simulation.to.estimate$e.hat <- e.hat
      
      # key step 
      simulation.to.estimate <- simulation.to.estimate[simulation.to.estimate$e.hat > 0.01,]
      simulation.to.estimate <- simulation.to.estimate[simulation.to.estimate$e.hat < 0.99,]
      
      
      bias.mu.1 <- mean(simulation.to.estimate$mu.hat.1-simulation.to.estimate$mu_1)
      bias.mu.0 <- mean(simulation.to.estimate$mu.hat.0-simulation.to.estimate$mu_0)
      bias.e <- mean(simulation.to.estimate$e.hat-simulation.to.estimate$e)
      
      term.A <- mean( (simulation.to.estimate$mu_1 - simulation.to.estimate$mu.hat.1) * (1 - (simulation.to.estimate$A /simulation.to.estimate$e))  ) 
      term.B <- mean( (simulation.to.estimate$Y_1 - simulation.to.estimate$mu_1) * ((1/simulation.to.estimate$e.hat) - (1/simulation.to.estimate$e))  )
      term.C <- mean( (simulation.to.estimate$e.hat-simulation.to.estimate$e) * (simulation.to.estimate$mu.hat.1-simulation.to.estimate$mu_1) )
